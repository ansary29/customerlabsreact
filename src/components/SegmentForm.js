...
function SegmentForm(props) {
  ...
  const addNewSchema = function () {
    ...
    setSelectedSchemas([
      ...selectedSchemas,
      ...schemas.filter((item) => item.Value === newSchema),
    ]);
  ...
  const changeSelectedSchema = function (idx, newValue) {
    ...
    setSelectedSchemas([
      ...selectedSchemas.slice(0, idx),
      ...newSchema,
      ...selectedSchemas.slice(idx + 1),
    ]);
    setSchemas([
      ...schemas.filter((item) => item.Value !== newValue),
      ...schemaRemoved,
    ]);
  };
  ...
  const handleSave = async function () {
    const url = "https://webhook.site/78f9b8fa-a028-4fc2-9d93-6024cbd57032";
    ...
    <div className="segment-form">
          ...
          {selectedSchemas.map((item, idx) => {
            return (
              <div className="segment-item">
                ...
                <select
                  key={idx}
                  value={selectedSchemas[idx].Value}
                  onChange={(event) =>
                    changeSelectedSchema(idx, event.target.value)
                  }
                >
                  <option key={99} value={item.Value}>
                    {item.label}
                  </option>
                  {schemas
                    .filter((item) => !selectedSchemas.includes(item.Value))
                    .map((item, idx) => (
                ...
                <button
                  ...
                  onClick={(event) => removeSchema(idx, event.target.value)}
                >
                  &nbsp;
                  <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
                  &nbsp;
      ...
      <div className="add-segment">
        <div className="segment-item">
          ...
          >
            <option key={99} value={DEFAULT_VALUE} disabled hidden>
              {DEFAULT_VALUE}
            </option>
            {schemas
              .filter((item) => !selectedSchemas.includes(item.Value))
      ...
      <div className="action-container">
        <div>
          <Button color="primary" className="btnSaveseg" onClick={handleSave}>
            Save the segment
          </Button>
        </div>
      </div>
